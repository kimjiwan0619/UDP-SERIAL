...
void CSerialTestSenderDlg::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 1000)
	{
		ZeroMemory(&m_sensorData, sizeof(stSensorData));
		SYSTEMTIME t;
		GetLocalTime(&t);
		CString strData;
		strData.Format(_T("%04d-%02d-%02d %02d:%02d:%02d.%04d"), t.wYear, t.wMonth, t.wDay, t.wHour, t.wMinute, t.wSecond, t.wMilliseconds);
		if (m_icnt == 0)
		{
			m_sensorData.SOP = 0x2599;
			m_sensorData.GID = 0x01;
			m_sensorData.SID = 0x01;
			m_sensorData.nTemperature = m_iCount1 - 40;
			memcpy(m_sensorData.strGPSDATA, strData, strData.GetLength());
			m_sensorData.EOP = 0x1241;
			if (m_b1) m_iCount1++;
			else m_iCount1--;
			if (m_iCount1 - 40 == 40) m_b1 = FALSE;
			if (m_iCount1 - 40 == -40) m_b1 = TRUE;
		}
		else if (m_icnt == 1)
		{
			m_sensorData.SOP = 0x2599;
			m_sensorData.GID = 0x02;
			m_sensorData.SID = 0x02;
			m_sensorData.nTemperature = m_iCount2 - 30;
			memcpy(m_sensorData.strGPSDATA, strData, strData.GetLength());
			m_sensorData.EOP = 0x1241;
			if (m_b2) m_iCount2++;
			else m_iCount2--;
			if (m_iCount2 - 30 == 30) m_b2 = FALSE;
			if (m_iCount2 - 30 == -30) m_b2 = TRUE;
		}
		else if (m_icnt == 2)
		{
			m_sensorData.SOP = 0x2599;
			m_sensorData.GID = 0x02;
			m_sensorData.SID = 0x03;
			m_sensorData.nTemperature = m_iCount3 - 20;
			memcpy(m_sensorData.strGPSDATA, strData, strData.GetLength());
			m_sensorData.EOP = 0x1241;		
			if (m_b3) m_iCount3++;
			else m_iCount3--;
			if (m_iCount3 - 20 == 20) m_b3 = FALSE;
			if (m_iCount3 - 20 == -20) m_b3 = TRUE;
		}
		else if (m_icnt == 3)
		{
			m_sensorData.SOP = 0x2599;
			m_sensorData.GID = 0x03;
			m_sensorData.SID = 0x04;
			m_sensorData.nTemperature = m_iCount4 - 10;
			memcpy(m_sensorData.strGPSDATA, strData, strData.GetLength());
			m_sensorData.EOP = 0x1241;
			if (m_b4) m_iCount4++;
			else m_iCount4--;
			if (m_iCount4 - 10 == 10) m_b4 = FALSE;
			if (m_iCount4 - 10 == -10) m_b4 = TRUE;
		}
		m_commThread.WriteComm((BYTE *)&m_sensorData, sizeof(stSensorData));
		m_icnt++;
		if (m_icnt == 4) m_icnt = 0;
	}
}
