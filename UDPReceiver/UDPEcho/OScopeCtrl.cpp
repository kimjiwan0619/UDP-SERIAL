...
void COScopeCtrl::AppendPoints(double dNewPoint[], int point, bool bInvalidate, bool bAdd2List, bool bUseTrendRatio)
{
	...
	//}
	if (bUseTrendRatio)
		m_PlotData[point].dCurrentPosition = (double)dNewPoint[point] / m_PlotData[point].iTrendRatio;
	else
	{
		m_PlotData[point].dCurrentPosition = dNewPoint[point];
	}
	if (bAdd2List)
	{
		m_PlotData[point].lstPoints.AddTail(m_PlotData[point].dCurrentPosition);
		while (m_PlotData[point].lstPoints.GetCount() > m_nMaxPointCnt)
			m_PlotData[point].lstPoints.RemoveHead();
	}
...
void COScopeCtrl::DrawPoint(int point)
{
